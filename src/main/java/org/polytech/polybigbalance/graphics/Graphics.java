...
public class Graphics
{
	...
	glfwSetWindowPos(screen, (vidmode.width() - Constants.WINDOW_WIDTH) / 2,
	...
	glTranslatef(rec.getCenterX() + ((int)rec.getWidth())*((sd.flipH)?1:0),
		rec.getCenterY() + ((int)rec.getHeight())*((sd.flipV)?1:0),
		0.0f);
	glScalef(rec.getWidth()/sd.rect.w*((sd.flipH)?-1.0f:1.0f),
		rec.getHeight()/sd.rect.h*((sd.flipV)?-1.0f:1.0f),
		0.0f);
	
	if (Math.abs(rec.getAngle()) > 0.001f && Math.abs(rec.getAngle() - Math.PI*2) > 0.001f)
		glRotatef((float) ((rec.getAngle()*180)/Math.PI), 0, 0, 1.0f);
	
	glTranslatef(-sd.rect.w*(1.5f - ((sd.flipH)?1.0f:0.0f)),
		-sd.rect.h*(1.5f - ((sd.flipV)?1.0f:0.0f)),
		0.0f);
	...
	//Render texture quad
	float x1 = ((float) rec.x) / surface.w;
	float x2 = ((float) rec.x + rec.w) / surface.w;
	float y1 = ((float) rec.y) / surface.h;
	float y2 = ((float) rec.y + rec.h) / surface.h;
    ...
    public Surface createSurface(BufferedImage image)
    {
	Surface surface = new Surface();
	surface.w = image.getWidth();
	surface.h = image.getHeight();
	surface.BytesPerPixel = image.getType();
	
	int[] pixels = new int[image.getWidth() * image.getHeight()];
        image.getRGB(0, 0, image.getWidth(), image.getHeight(), pixels, 0, image.getWidth());

        ByteBuffer buffer = BufferUtils.createByteBuffer(image.getWidth() * image.getHeight() * surface.BytesPerPixel);
	...
	surface.texture = loadTextureGL(surface);
	
	return surface;
    }
    
    public int loadTextureGL(Surface image)
    {
	...
	switch(image.BytesPerPixel)
	{
		...
		case 1:
			...
			break;
		default:
			System.out.println("not supported bytes per pixel " + image.BytesPerPixel);
			break;
	}
	glBindTexture(GL_TEXTURE_2D, 0);
	return textures;
